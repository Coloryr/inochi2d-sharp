...
public record PuppetMeta
{
    ...
    /// <summary>
    /// Whether the puppet should preserve pixel borders.<br/>
    ...
    public void Deserialize(JsonElement data)
    {
        ...
        {
            if (item.Name == "name" && item.Value.ValueKind is JsonValueKind.String)
            {
                Name = item.Value.GetString()!;
            }
            else if (item.Name == "version" && item.Value.ValueKind is JsonValueKind.String)
            {
                Version = item.Value.GetString()!;
            }
            else if (item.Name == "rigger" && item.Value.ValueKind is JsonValueKind.String)
            {
                Rigger = item.Value.GetString()!;
            }
            else if (item.Name == "artist" && item.Value.ValueKind is JsonValueKind.String)
            {
            ...
            else if (item.Name == "copyright" && item.Value.ValueKind is JsonValueKind.String)
            {
                Copyright = item.Value.GetString()!;
            }
            else if (item.Name == "licenseURL" && item.Value.ValueKind is JsonValueKind.String)
            {
                LicenseURL = item.Value.GetString()!;
            }
            else if (item.Name == "contact" && item.Value.ValueKind is JsonValueKind.String)
            {
                Contact = item.Value.GetString()!;
            }
            else if (item.Name == "reference" && item.Value.ValueKind is JsonValueKind.String)
            {
