...
public record GLTexture : IDisposable
{
    ...
    public GLTexture(GlApi gl, uint color, uint width, uint height)
    {
        _gl = gl;
        _color = color;
        _width = width;
        ...
        gl.TexImage2D(GlApi.GL_TEXTURE_2D, 0, color, width, height, 0, color, GlApi.GL_UNSIGNED_BYTE, 0);
    ...
    public unsafe GLTexture(GlApi gl, uint color, uint width, uint height, byte[] data)
    {
        _gl = gl;
        _color = color;
        _width = width;
        ...
        fixed (void* ptr = data)
        {
            gl.TexImage2D(GlApi.GL_TEXTURE_2D, 0, color, width, height, 0, color, GlApi.GL_UNSIGNED_BYTE, new IntPtr(ptr));
        }
        gl.TextureParameteri(TexId, GlApi.GL_TEXTURE_MIN_FILTER, GlApi.GL_LINEAR);
        gl.TextureParameteri(TexId, GlApi.GL_TEXTURE_MAG_FILTER, GlApi.GL_LINEAR);
        gl.TextureParameteri(TexId, GlApi.GL_TEXTURE_WRAP_S, GlApi.GL_CLAMP_TO_BORDER);
